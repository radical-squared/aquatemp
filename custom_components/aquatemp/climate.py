...
class Aquatemp(ClimateEntity):
    ...
    def __init__(self, config):
        ...
        self._temperature_unit = (
            TEMP_CELSIUS if config["temperature_unit"] == "C" else TEMP_FAHRENHEIT
        )
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        async with aiohttp.ClientSession(SERVER_URL) as session:
            async with session.post(
                CONTROL_PATH, headers=self._headers, json=data
            ) as response:
                if response:
            ...
            async with aiohttp.ClientSession(SERVER_URL) as session:
                async with session.post(
                    ...
                    if response.status:
                        ...
                        if response_json["error_msg"] == "Success":
                            self._attributes["power"] = (
                                "off" if hvac_mode == HVAC_MODE_OFF else "on"
                            )
    ...
    async def async_update(self):
        ...
        for code in self._attribute_map:
            if self.get_value(code) is not None:
                self._attributes[self._attribute_map[code]] = float(
                    self.get_value(code)
                )
    ...
    async def fetch_data(self):
        ...
        async with aiohttp.ClientSession(SERVER_URL) as session:
            async with session.post(
                GETDATABYCODE_PATH, headers=self._headers, json=data
            ) as response:
                if response.status == 200:
                    response_json = await response.json()
                    _LOGGER.debug(response_json)
        ...
        async with aiohttp.ClientSession(SERVER_URL) as session:
            async with session.post(
                ...
                if response:
                    response_json = await response.json()
                    _LOGGER.debug(response_json)
                    self._attributes["is_fault"] = bool(
                        response_json["object_result"]["is_fault"]
                    )
                else:
                    await self.login(session)
            ...
            if self._attributes["is_fault"] == True:
                async with session.post(
                    ...
                    if response:
                        response_json = await response.json()
                        _LOGGER.debug(response_json)
                        self._attributes["fault"] = response_json["object_result"][0][
                            "description"
                        ]
                    else:
                        await self.login(session)
    ...
    async def login(self, session):
        ...
        async with session.post(
            LOGIN_PATH, headers=self._headers, json=data
        ) as response:
            response_json = await response.json()
            _LOGGER.debug(response_json)
